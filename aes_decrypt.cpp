...
long long int GetFileSize2(string filename) {
	struct stat stat_buf;

	long long int rc = stat(filename.c_str(), &stat_buf);
...
void SubBytes2(unsigned char* state) {
	for (int i = 0; i < 16; i++) {
		state[i] = inv_s[state[i]];
	}
}
void ShiftRows2(unsigned char* state) {
...
void MixColumns2(unsigned char* state) {
	unsigned char tmp[16];
...
void AddRoundKey2(unsigned char* state, unsigned char* roundKey) {
	for (int i = 0; i < 16; i++) {
		state[i] ^= roundKey[i];
	}
}
...
void Decrypt(unsigned char* message, unsigned char* key, unsigned char* decryptedMessage) {
	...
	AddRoundKey2(state, expandedKey + 160);
	ShiftRows2(state);
	SubBytes2(state);


	for (int i = 8; i >= 0; i--) {
		AddRoundKey2(state, expandedKey + (16 * (i + 1)));
		MixColumns2(state);
		ShiftRows2(state);
		SubBytes2(state);

	}
	AddRoundKey2(state, key);
...
void DECRYPT_FUNCTION() {
	...
	ifstream inFile("encryptedMessage.valknut", ios::in | ios::binary);
	ofstream outFile("decrypted.txt", ios::binary);

	string inputFilename = "encryptedMessage.valknut";
	...
	while (encryptedDataLength < decryptionLength) {
		...
		*/
		...
		long long int messageLen = 4096;
		unsigned char* decryptedMessage = new unsigned char[messageLen];

		for (long long int i = 0; i < messageLen; i += 16) {
